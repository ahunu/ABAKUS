...
function REGELN(pCup) {
    ...
    if (pCup === 55) { // Tirolcup
        return '<div class="J">'
                ...
                + "<tr><th>&nbsp;&nbsp;Besserrufer I, II</th><th class=TR>1&nbsp;&nbsp;</th></tr>"
                ...
                + "<tr><th>&nbsp;&nbsp;Besserrufer III, IIII</th><th class=TR>1&nbsp;&nbsp;</th></tr>"
                ...
                + '<li><b>Königrufen</b> mit 54 Karten wird gegen den Uhrzeigersinn gespielt. Es muss abgehoben werden. Jeder Spieler erhält zweimal sechs Karten. Mit jedem Blatt muss gespielt werden. In positiven Spielen spielt die Vorhand zum ersten Stich aus, in negativen der Spieler. Es gilt Farbzwang (man muss Farbe bekennen) und Tarockzwang (eine Fehlfarbe muss mit Tarock bedient werden).</li>'
                + '<li><b>Kaiserstich</b>: Fallen Mond, Sküs und Pagat in dieser Reihenfolge, sticht der Pagat.</li>'
                + '<li><b>Was liegt, pickt.</b> Eine regulär gespielte Karte darf nicht zurückgenommen werden, außer bei Renonce (solange der Stich noch nicht umgedreht oder bereits zum nächsten Stich ausgespielt wurde). Jeder darf den letzten Stich sehen, alle anderen Stiche und der Talon sind tabu.</li>'
                ...
                + '<li><b>Bieten:</b> Die Vorhand beginnt mit Vorhand oder einer Spielansage. Die Vorhandspiele stehen nur der Vorhand offen. Der Sechserdreier kann nur vorneweg angesagt werden, alle anderen Spiele auch im Nachhinein. Angesagte Spiele dürfen nur durch höherrangige Spiele überboten werden. Wer einmal gepasst hat, darf nicht mehr mitbieten.</li>'
                + '<li><b>Rufen:</b> Bei Partnerspielen wird jetzt der König gerufen. Hat man drei Könige im Blatt, kann man den Vierten rufen (ohne die Farbe zu nennen). Sich selber rufen ist nicht erlaubt. Liegt der gerufene König im Talon, kann man das Spiel schleifen (aufgeben); die Hälfte mit dem König darf man liegenlassen.</li>'
                ...
                + '<li><b>Kontras:</b> verdoppeln jeweils den Spielwert (Kontra/Re/Sub). Spiel und Zusätze werden getrennt kontriert. Alle Ansagen dürfen nur bei erster Gelegenheit kontriert werden (kein nachschießen). In Negativspielen kontriert jeder für sich, ansonsten gilt ein Kontra immer für alle Spieler.</li>'
                + '</ul>'
                ...
                + '<li><b>Farbenspiele:</b> Tarock sticht nicht. Solange man Farbkarten hat, darf man kein Tarock ausspielen. Kann man eine Farbe nicht bedienen, muss man Tarock zugeben. Statt der Farbkarten sind Tarock zu verlegen (verdeckt; Farbkarten nur erzwungen und offen). Gültige Zusätze: Könige, Säcke und Valat.</li>'
                + '</ul>'
                ...
                + '<li><b>Ouvertspiele:</b> Die Karten werden nach dem ersten Stich aufgedeckt. Üblicherweise wird mit Reden gespielt, die Gegenspieler dürfen sich über die Spielstrategie unterhalten.</li>'
                + '</ul>'

                + '<ul>'
                + '<div class="M B C" style="padding: 1vh 0 1vh 0">5) Trischaken</div>'
                + '<li>Bunt gemischt. Die Vorhand kann bestimmen, ob mit oder ohne Stichzwang gespielt wird und ob der Talon zu den ersten sechs Stichen oder zum letzten Stich dazugegeben wird. Es wird blattgenau abgerechnet.</li>'
                ...
                + '<li>Spiel und Prämien werden getrennt verrechnet. Jeder Spieler kann Zusätze ansagen oder still spielen.</li>'
                + '<li><b>Vögel und König ultimo:</b> Die Vögel müssen selber stechen, bei König Ultimo genügt es auch, wenn der Partner sticht. Vögel und König Ultimo kann man nur ansagen, wenn man die jeweilige Karte im Blatt hat (kein kommandieren). Beide Ansagen dürfen nicht freiwillig aufgegeben werden. Müssen angesagte Vögel vorzeitig gespielt werden, muss immer der höchste Vogel zuerst aufgegeben werden.</li>'
                + '<li>Die fixe <b>Valatprämie</b> ist so wie die anderen Zusätze abzurechnen, d. h. zusätzlich zum Spiel und in Solospielen doppelt. Im Falle eines Valats zählen alle Verlaufsprämien (Vögel, Fänge, Ultimo), aber keine Bestandsprämien (Trull, Könige, Säcke).</li>'
                + '</ul>'
                ...
                + '<li>Üblicherweise werden drei Runden mit je 16 Spielen ausgetragen. Der Turnierleiter kann bei Bedarf ausständige Tische vorzeitig beenden. Die Sitzordnung wird durch Kartenziehen festgelegt (Tarock ist erste Vorhand). Pro Punkt werden zehn Cent abgerechnet. Kontras zählen nicht für die Turnierwertung.</li>'
                + '<li>Die Schriftpunkte aus allen Runden werden addiert. Der Spieler mit den meisten Punkten gewinnt. Bei Punktegleichheit zählen die besseren Rundenergebnisse und dann das Los.</li>'
                + '</ul>'

                + '<ul>'
                + '<p class=S>(c) 2020 Markus Mair, www.tarock.tirol</p>'
                + '</ul>'
                + '</div>';
        ...
        return '<div class="J">'
                ...
                + "<tr><th>&nbsp;&nbsp;Valat</th><th class=TR>4-/8-fach&nbsp;&nbsp;</th></tr>"
                + "</tbody></table>"
                + '<br>'
                + "*) Vorhandspiele"
                + '</div></div>'
                ...
                + '<li>Es muss mit Geld gespielt werden (1 Punkt = 10 Cent), und es wird plus und minus geschrieben. Wer die höchste Karte hebt, beginnt mit der Abgabe der Karten. Es muss gründlich gemischt und abgehoben werden. Der Talon wird in der Mitte gegeben. Jedes Spiel muss gespielt bzw. gewertet werden. Ein Blatt ohne Tarock und Könige darf nicht abgelegt werden.</li>'
                + '<li>Zum Gewinn braucht ein Spieler 36 Punkte (35 Punkte und 2 Blatt).</li>'
                + '<li>Nur mit „Vorhand“ kann sich der Vorhandspieler freihalten, ob er ein positives oder negatives Spiel spielt. Alle Spiele außer Sechserdreier sind möglich, auch ohne vorherige Lizitation. Ein Spiel wird mit "ich liege" eröffnet. Die übrigen Spieler müssen jede Ansage mit "gut" zur Kenntnis nehmen - außer sie kontrieren.</li>'
                + '<li>Bei den Spielen Rufer, Solorufer und A-Rufer ist es untersagt einen König zu rufen, der sich im eigenen Blatt befindet. Hat man drei Könige im Blatt, ist es erlaubt den „vierten König“ zu rufen, ohne die Farbe zu verraten. Liegt beim Rufer oder A-Rufer der gerufene König im Talon, kann der Spieler das Spiel „schleifen“. Es werden die Punkte des normalen Rufers bzw. A-Rufers + Ansage gewertet.</li>'
                ...
                + '<li>Der Vorhandspieler hat einen Sechserdreier sofort anzumelden, alle anderen Spiele können auch im Nachhinein gemeldet werden. Wird der Sechserdreier verloren, wird nur das Spiel (nicht aber die Ansagen) doppelt gewertet (der verlorene Valat beim Sechserdreier zählt 8 x 8 Punkte).</li>'
                + '<li>Ein weiteres Recht des Vorhandspielers ist es, ein bereits lizitiertes Spiel zu halten (also selbst zu spielen).</li>'
                + '<li>Hat der Vorhandspieler alle vier Könige im Blatt und sieht er keine Möglichkeit ein Spiel zu spielen, muss er die Vorhand an den nächsten Spieler mit "ich gebe die Vorhand weiter" abgeben. Alle Rechte der Vorhand gehen somit auf den nächsten Spieler über, auch das Recht einen Sechserdreier zu spielen. Auf das nächste Spiel hat dies keine Auswirkung, d. h. ein Spieler hat zweimal hintereinander die Vorhand.</li>'
                + '</ul>'

                + '<ul>'
                + '<div class="M B" style="padding: 1vh 0 1vh 0">Trischaken:</div>'
                + '<li>Es besteht Farb- und Stichzwang, aber es muss nicht austarockiert werden. Der Pagat darf nur als letzter Tarock aus- bzw. zugegeben werden. Der Spieler, der den letzten Stich erzielt, erhält den Talon.</li>'
                ...
                + '<li>Bei einer Renonce erhalten die drei regulären Spieler immer jeweils drei Punkte, der Renoncespieler neun Minuspunkte (unabhängig davon, wer das Spiel gespielt hat).</li>'
                + '</ul>'

                + '<ul>'
                + '<div class="M B" style="padding: 1vh 0 1vh 0">Negativspiele:</div>'
                + '<li>Negativspiele sind: Piccolo, Bettel, Piccolo-Ouvert, Bettel-Ouvert. Nicht die Vorhand, sondern der Spieler selbst spielt aus. Der Pagat darf nur als letzter Tarock aus- bzw. zugegeben werden. Es herrscht Stichzwang.</li>'
                + '<li>Bei den Ouvertspielen werden nach dem ersten und vor dem zweiten Ausspielen alle Karten offen auf den Tisch gelegt. Die drei Gegenspieler dürfen sich untereinander über die Spiestrategie beraten.</li>'
                + '</ul>'

                + '<ul>'
                + '<div class="M B" style="padding: 1vh 0 1vh 0">Farbensolo:</div>'
                + '<li>Hier sind Tarock als „5. Farbe“ zu sehen, man muss mindestens 6 Farbkarten im Blatt haben. Die Vorhand spielt aus. Es darf erst Tarock gespielt werden, wenn man keine Farbkarten mehr im Blatt hat.</li>'
                ...
                + '<li>Beim Solodreier (höchstes Spiel, daher kein Überbieten mehr möglich) müssen alle Ansagen sofort gemeldet werden, bevor der Spieler das Spiel für eröffnet erklärt (z.B. mit „ich liege“).</li>'
                + '<li>Die Figuren Pagat, Uhu, Kakadu, Quapil sowie König Ultimo können nicht für andere Spieler (z.B. Partner) angesagt werden. Eine der vier Figuren Pagat, Uhu, Kakadu oder Quapil ist Voraussetzung für einen A-Rufer. Hat man keine dieser Figuren vor dem Talonkauf im Blatt und spielt trotzdem einen A-Rufer, so ist dies als Renonce zu werten (je drei Punkte an die Mitspieler).</li>'
                ...
                + '<li>Wird ein angesagter Vogel oder König zum falschen Zeitpunkt gespielt, so ist er verloren (darf nicht mehr zurückgenommen werden), aber es ist keine Renonce, sofern der Spieler nicht gegen die Reihenfolge (siehe oben) verstößt. Der Spielpartner muss den Verlust mittragen. Will sich jedoch ein Spieler offensichtlich einen Vorteil verschaffen (etwa angesagten Pagat oder König ultimo vorzeitig heimstechen um Trull oder Spiel zu retten), so ist das Renonce.</li>'
                ...
                + '<li>Alle regulären Spieler erhalten die Punkte als hätten sie das Spiel und alle angesagten Ansagen gewonnen (Ausnahme: Trischaken, siehe dort). Der Spieler, der die Renonce begangen hat, erhält die Punkte der drei Mitspieler als Minuspunkte eingetragen.</li>'
                ...
                + '<li>Falsches Ausspielen: Zurechtweisen von allen Spielern aber keine Renonce (wiederholt falsches Ausspielen wird nach Ermahnung als Renonce gewertet). Nach eingezogenen Karten, keine Reklamationen mehr möglich und das Spiel läuft ungestraft weiter.</li>'
