...
//            return '-';
    ...
    stStat = 'Steirerwertung';
    stNamenLen = 0.3;
    writeCanvas('Steirerwertung  ' + stSaison);
    ...
    for (var spieler in SP) { // der Internet Explorer versteht kein  for (var CUPrec of CUP)
        var hCuppunkte = 0;
        if (CUP[spieler]) {
            for (i = 0; i < CUP[spieler].length; i++) {
        ...
        if (SP[spieler][iSaison]) {
            SORTnachPlatz.push((5000 - hCuppunkte) + (SPIELER[spieler] ? SPIELER[spieler][0] : '????') + ';' + spieler);
        }
    }
...
function showHeinewertung() {
    ...
    for (var spieler in SP) { // der Internet Explorer versteht kein  for (var CUPrec of CUP)
        var hCuppunkte = 0;
        if (stFinale) {
            if (IsInteger(getCupPunkte(stFinale, spieler))) {
