...
//    cChart.chart.spacingLeft = 30;
    ...
    cChart.plotOptions= { column:  { dataLabels: { enabled: true, formatter: function () {if (this.y >= 10) {return this.y + ' %';} else {return this.y;}}, y: 10, align: 'right', rotation: -90, style: { color: '#eee', fontSize: '20px', fontFamily: 'Verdana, sans-serif' }}}};
...
//          cChart.xAxis.categories[ii] = '<b>'+LS.Spieler[j]+'<br>'+prozGespielt[j]+'</b>, '+Math.round((eigGewonnen[j])/((eigGewonnen[j]+eigVerloren[j])/100))+' %';
            cChart.series[0].data[ii] = prozGespielt[j];
        } else {
//          cChart.xAxis.categories[ii] = '<b>'+LS.Spieler[j]+'<br>0</b> Spiele';
